...
import com.ksw.dao.forObject.relation.AnnouncementUserMapper;
...
public class AnnouncementService {
	...
	/*
	 * public Announcement insertAnnouncement(AnnouncementUser a) {
	 * announcementUserMapper.insert(a); }
	 */
	...
	public AnnouncementDTO convertToDTO(Announcement announcement) {
		...
		if (announcement == null) {
			System.out.println(
					"Announcement to AnnouncementDTO failed. Empty AnnouncementDTO created. Announcement is null");
			return dto;
		}
	...
	public AnnouncementVO convertToVO(AnnouncementDTO announcementDTO) {
		if (announcementDTO == null) {
			System.out.println(
					"AnnouncementDTO to AnnouncementVO failed. Empty AnnouncementVO created. AnnouncementDTO is null");
			return new AnnouncementVO.Builder().build();
		}
		return new AnnouncementVO.Builder().announcementNo(announcementDTO.getAnnouncementNo())
				.announcementTitle(announcementDTO.getAnnouncementTitle())
				.announcementContent(announcementDTO.getAnnouncementContent()).schedule(announcementDTO.getSchedule())
				.isActive(announcementDTO.getIsActive()).createdAt(announcementDTO.getCreatedAt())
				.updatedAt(announcementDTO.getUpdatedAt()).build();
	}
