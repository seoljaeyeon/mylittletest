...
public interface CategoryUserMapper {
	...
	@Insert(""
			+ "INSERT INTO categoryUser "
			+ "(categoryNo, noteNo) "
			+ "VALUES "
			+ "(#{category.categoryNo}, #{note.noteNo})")
	void insert(CategoryUser categoryUser);
	
    @Select("SELECT noteNo FROM categoryUser "
    ...
    CategoryUser getCategoryUserbynoteNo(Integer noteNo);
    
    @Select(""
    		+ "SELECT c.* "
    		+ "FROM category c JOIN categoryUser nc "
    ...
    @Select("("
            + "SELECT nu.noteNo "
            + "FROM noteUser nu "
            + "JOIN categoryUser nc ON nu.noteNo = nc.noteNo "
